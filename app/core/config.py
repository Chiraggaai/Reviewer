...
class Settings(BaseSettings):
    ...
    # ── Reviewer invitation emails (SMTP) ─────────────────────────────────
    # When True or SMTP_HOST is empty, invitation content is logged instead of sent.
    INVITATION_EMAIL_DRY_RUN: bool = True
    SMTP_HOST: Optional[str] = None
    SMTP_PORT: int = 587
    SMTP_USER: Optional[str] = None
    SMTP_PASSWORD: Optional[str] = None
    SMTP_FROM: str = "noreply@glimmora.local"
    ...
    # ── Contributor module (``app/contributor``) ───────────────────────────
    # When True, ``X-Contributor-Id`` works without JWT on contributor routes (dev only).
