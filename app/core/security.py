...
async def require_contributor_user(
    ...
    """
    Contributor-only routes (same auth stack as enterprise: Bearer access token + MFA policy).

    Use: ``current_user: dict = Depends(require_contributor_user)``.
    Enterprise and reviewer tokens receive 403.
    """
...
_PLATFORM_ROLES = frozenset({"enterprise", "contributor", "reviewer"})
...
async def require_platform_user(
    ...
    """
    Reviewer API: allow enterprise, contributor, or reviewer (Bearer access token from Glimmora-Team-Project auth).
    """
    role = str(current_user.get("role") or "").strip().lower()
    if role not in _PLATFORM_ROLES:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "This resource requires an enterprise, contributor, or reviewer account.",
            },
        )
    return current_user
